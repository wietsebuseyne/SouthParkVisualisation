function wordcloud() {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      wordTooltip = d3.select(this)
              .append("div")
              .attr("class", "hoverInfo tiny");
      ...
      if(!isEpisodeWordcloud())
        ...
        data.unshift({text:clearText, count:sizeScale.domain()[1]});
      d3.layout.cloud()
  ...
  function draw(words) {
    ...
    words.forEach(function(d) {
      if(d.text != "all" && d.text != clearText)
        allCount += d.count;
    });
    ...
    cloud.enter()
      .append("text")
        ...
        cloud.on("mouseover", function(d) {
          wordTooltip.html(function() {
            if(d.text === clearText) return ("Toggle selection for all words");
            return "Occurences: " + (d.text === "all" ? allCount : d.count);
          });
        ...
        cloud.on('mousemove', function() {
          wordTooltip.style('top', (d3.event.layerY - 20) + 'px')
    ...
    if(isEpisodeWordcloud())
      cloud
        .on("click", function(d) {
            if(d.text === clearText) {
              if(shownWords.length > 0) { //clear all
                for(var i = 0; i < words.length; i++) {
                  if(shownWords.indexOf(words[i].text) != -1)
                    barcodeGraph.toggle(words[i].text);
                  wordShown[words[i].text] = false;
                }
                shownWords.splice(0, shownWords.length);
                d3.selectAll(".word").attr("class", "word clickableText unselected");
                d3.select(this).text(allText);
              } else { //select all
                for(var i = 0; i < words.length; i++) {
                  if(shownWords.indexOf(words[i].text) == -1) {
                    barcodeGraph.toggle(words[i].text);
                    shownWords.push(words[i].text);
                  }
                  wordShown[words[i].text] = true;
                }
                d3.selectAll(".word").attr("class", function(d) { 
                  if(d.text === clearText)
                    return "word clickableText unselected";
                  return "word clickableText";
                });
                d3.select(this).text(clearText);
              }
            } else {
  ...
  function loadLineGraphData(words) {
    //Don't draw if changed to episode wordle
    if(secondaryInfo == 1)
      return;
