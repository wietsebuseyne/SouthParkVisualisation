...
def main(argv):
    ...
    output = [["character\tvalue\tprofanities\tline\tprofanityList"] for _ in range(0,257)]
    ...
    for line in subtitles:
        ...
        parts = line.split('\t')
        season = int(parts[0])
        episode = seasonEpisodeToAbsoluteEpisode(season, int(parts[1]))-1
        character = parts[2]
        #if character not in characters:
        #    character = 'Other'

        sentence = parts[3].translate(None, '"')
        ...
        for word in words:
            ...
            if w in profanities:
                ...
                profanityList.append(w)
            amount_of_words += 1
        # end-for

        if amount_of_words > 0:
            output[episode].append(character + '\t' + str(amount_of_words) + '\t' + str(amount_of_profanities) + '\t' + sentence.strip() + '\t' + str(profanityList).lstrip() )
